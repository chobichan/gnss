...
---------------------------------------- */
GNSS_BY_SOFT::GNSS_BY_SOFT()
{
}

GNSS_BY_SOFT::GNSS_BY_SOFT( uint32_t rate, uint8_t rPin, uint8_t tPin )
{
  begin( rate, rPin, tPin );
}
...
/* ----------------------------------------
    begin and end.
---------------------------------------- */
void GNSS_BY_SOFT::begin( uint32_t rate, uint8_t rPin, uint8_t tPin )
{
  brr = rate;
  rxdPin = rPin;
  txdPin = tPin;
  SoftwareSerial::begin( brr, SWSERIAL_8N1, rxdPin, txdPin, false , 4096, 4096 ); // RX,TX
}

/* ----------------------------------------
    GPS sentence monitor.
  ---------------------------------------- */
void GNSS_BY_SOFT::monitor()
{
  while( 1 )
  {
    if( Serial.available() )
...
/* ----------------------------------------
    get one line fron GPS serial.
  ---------------------------------------- */
int GNSS_BY_SOFT::line( char *rcv, int size )
{
  int count = 0;
  /* get line. */
  *rcv = '\0';
...
float GNSS::latitude( const char *str )
{
  ...
  tempI += (str[1] - '0');
  float tempF = char2Float( (const char *)&str[2] );
...
//  Serial.println( "argc=" + (String)argc );
  ...
  /* day,month,year */
  rmc->ddmmyy = (uint32_t)char2Int( (const char *)argv[10] );
  /* magnetic deviation */
  rmc->magneticDeviation = char2Float( (const char *)argv[11] );
  /* positional mode */
